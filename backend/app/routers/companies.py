...
@router.post("/search")
def search_companies(payload: SearchRequest, db: Session = Depends(get_db)):
    ...
    if payload.filters.import_id and payload.filters.import_id.strip():
        query = query.filter(Company.import_id == payload.filters.import_id.strip())
...
def list_companies_by_import_all(import_id: str, db: Session = Depends(get_db)):
    items = (
        db.query(Company)
